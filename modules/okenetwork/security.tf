...
# mysql security list
resource "oci_core_security_list" "mysql_seclist" {
  compartment_id = var.compartment_id
  display_name   = var.label_prefix == "none" ? "mysql" : "${var.label_prefix}-mysql"
  vcn_id         = var.oke_network_vcn.vcn_id

  dynamic "egress_security_rules" {
    iterator = mysql_egress_iterator
    for_each = local.mysql_egress

    content {
      description      = mysql_egress_iterator.value["description"]
      destination      = mysql_egress_iterator.value["destination"]
      destination_type = mysql_egress_iterator.value["destination_type"]
      protocol         = mysql_egress_iterator.value["protocol"]
      stateless        = mysql_egress_iterator.value["stateless"]

      dynamic "tcp_options" {
        for_each = mysql_egress_iterator.value["protocol"] == local.tcp_protocol && mysql_egress_iterator.value["port"] != -1 ? [1] : []

        content {
          min = mysql_egress_iterator.value["port"]
          max = mysql_egress_iterator.value["port"]
        }
      }

      dynamic "icmp_options" {
        for_each = mysql_egress_iterator.value["protocol"] == local.icmp_protocol ? [1] : []
  ...
  dynamic "ingress_security_rules" {
    iterator = mysql_ingress_iterator
    for_each = local.mysql_ingress

    content {
      description = mysql_ingress_iterator.value["description"]
      protocol    = mysql_ingress_iterator.value["protocol"]
      source      = mysql_ingress_iterator.value["source"]
      stateless   = mysql_ingress_iterator.value["stateless"]

      dynamic "tcp_options" {
        for_each = mysql_ingress_iterator.value["protocol"] == local.tcp_protocol && mysql_ingress_iterator.value["port"] != -1 ? [1] : []

        content {
          min = mysql_ingress_iterator.value["port"]
          max = mysql_ingress_iterator.value["port"]
        }
      }

      dynamic "icmp_options" {
        for_each = mysql_ingress_iterator.value["protocol"] == local.icmp_protocol ? [1] : []

...
locals {
  ...
  mysql_subnet  = cidrsubnet(var.oke_network_vcn.vcn_cidr, var.oke_network_vcn.newbits["mysql"], var.oke_network_vcn.netnum["mysql"])

  mysql_egress = [
    {
      description      = "Allow egress for all traffic to allow pods to communicate between each other on different mysql nodes on the mysql subnet",
      destination      = local.mysql_subnet,
      destination_type = "CIDR_BLOCK",
      protocol         = local.all_protocols,
      port             = -1,
      stateless        = false
    ...
    {
      description      = "Allow mysql nodes access to Internet. Required for getting container images or using external services",
      destination      = local.anywhere,
      destination_type = "CIDR_BLOCK",
      protocol         = local.tcp_protocol,
      port             = -1,
  ...
  mysql_ingress = [
    {
      description = "Allow ingress for all traffic to allow pods to communicate between each other on different mysql nodes on the mysql subnet",
      protocol    = local.all_protocols,
      port        = -1,
      source      = local.mysql_subnet,
      stateless   = false
    },
    {
      description = "Allow workers to communicate with mysql nodes",
      protocol    = local.tcp_protocol,
      port        = -1,
      source      = local.worker_subnet,
      stateless   = false
    ...
    {
      description = "Allow path discovery from mysql nodes"
      protocol    = local.icmp_protocol,
      port        = -1,
      source      = local.anywhere,
      stateless   = false
